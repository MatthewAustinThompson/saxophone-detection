...
from highpass import apply_butter_high
...
OUT_DIR = './data/5s/labeled/features_r03'
    ...
    for fn in os.listdir(sample_dir):
        if fn != TEMP_FILENAME:
            ...
            if ext == '.wav':
                ...
                # First apply highpass filter and save to temp file
                rate, data = scipy.io.wavfile.read(in_file)
                assert rate == 44100
                data = data.astype('float64')
                fs = rate
                filtered = apply_butter_high(data, CUTOFF, fs, order=ORDER)
                # ...and do it AGAIN. Two passes.
                # filtered = apply_butter_high(filtered, CUTOFF, fs, order=ORDER)
