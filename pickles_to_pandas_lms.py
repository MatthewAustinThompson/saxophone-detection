...
def lms_pickles_to_pandas(dir_=DIR):
    ...
    for f in os.scandir(dir_):
        ...
        if f.path.endswith(".lms"):
            counter +=1
            # print(f.name)
            name = f.name.replace('_PICKLE.lms', '.wav')
            ...
            with open(os.path.join(f.path), "rb") as pfh:
                ...
                for band_num, lms_band in enumerate(pdata):
                    for t, val in enumerate(lms_band):
                        if first:
                            col_names.append('lms_{}_{}'.format(band_num, t))
                        rrow.append(val)
                if first:
            ...
            if not counter % 1000:
                ...
                df.to_pickle('./data/5s/labeled/features_r02/lms_data_{}.pkl'
